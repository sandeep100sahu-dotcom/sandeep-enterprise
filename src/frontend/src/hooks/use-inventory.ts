...
const SEED_GRNS: GRNEntryPublic[] = [
  {
    id: "1",
    grnNo: "GRN-2025-0041",
    supplierId: "sup1",
...
const SEED_ISSUES: IssueEntryPublic[] = [
  {
    id: "1",
...
export function useGRNs() {
  return useQuery<GRNEntryPublic[]>({
    queryKey: ["grns"],
...
export function useAddGRN() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (grn: Omit<GRNEntryPublic, "id" | "createdAt">) =>
      ({
        ...grn,
...
export function useUpdateGRNQCStatus() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, status }: { id: string; status: QCStatus }) => ({
      id,
      status,
    }),
    onSuccess: () => {
...
export function useIssues() {
  return useQuery<IssueEntryPublic[]>({
    queryKey: ["issues"],
...
export function useAddIssue() {
  ...
  return useMutation({
    mutationFn: async (
      issue: Omit<IssueEntryPublic, "id" | "createdAt" | "status">,
    ) =>
      ({
        ...issue,
        id: Date.now().toString(),
